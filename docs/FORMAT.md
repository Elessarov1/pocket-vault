...
В этих четырёх значениях DeviceStorage никакие секретные ключи не сохраняются
отдельно от контейнера. Необязательный кэш быстрого входа хранится вне формата V1
в Telegram SecureStorage; он никогда не записывается в DeviceStorage или
localStorage. Кэш содержит собственную версию, время последнего явного ввода и
границу ближайшей локальной полуночи; автоматическое открытие её не продлевает.
Разблокированная сессия владеет случайным DEK.
...
1. `password_key = Argon2id(master_password, kdf.salt, m, t, p, 32)` с версией
   Argon2 `0x13`.
2. `KEK = HKDF-SHA-256(password_key, salt=vault_id,
   info="pocket-vault/local-kek/v1", L=32)`.
3. Случайный 32-байтовый DEK зашифрован KEK и хранится в `wrappedDek`.
4. Контейнеры обоих слотов шифруются DEK.
...
Мастер-фраза — единственный пользовательский секрет. Получив копию metadata и
слотов, атакующий может проверять догадки офлайн, поэтому интерфейс требует не
менее 16 символов и рекомендует длинную уникальную фразу.

При смене мастер-фразы существующий DEK оборачивается новым KEK с новой случайной
солью. Меняются только `kdf` и `wrappedDek` в metadata; `vaultId`, `createdAt` и
зашифрованные слоты остаются прежними.
