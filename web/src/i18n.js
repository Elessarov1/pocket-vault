...
const ENGLISH = new Map(Object.entries({
  ...
  "Для быстрого входа мастер-пароль может храниться в защищённом хранилище устройства. Он не отправляется разработчику.": "For quick unlock, the master password may be kept in the device’s secure storage. It is never sent to the developer.",
  "Понятно": "Got it",
  ...
  "После смены для быстрого входа будет сохранён только новый мастер-пароль.": "After the change, only the new master password will be remembered for quick unlock.",
  ...
  "Зашифрованный контейнер хранится локально через Telegram DeviceStorage. На поддерживаемых мобильных клиентах мастер-пароль может отдельно сохраняться в Telegram SecureStorage для быстрого входа. Он не записывается в DeviceStorage или localStorage. Pocket Vault не имеет собственного сервера, аккаунтов и облачной синхронизации.": "The encrypted container is stored locally through Telegram DeviceStorage. On supported mobile clients, the master password may be stored separately in Telegram SecureStorage for quick unlock. It is not written to DeviceStorage or localStorage. Pocket Vault has no server, accounts, or cloud sync.",
  ...
  "Мастер-пароль не отправляется разработчику. Если клиент поддерживает быстрый вход, он хранится средствами Telegram SecureStorage, использующими защищённое хранилище устройства. Ручная блокировка отключает быстрый вход до следующего явного ввода. Без мастер-пароля или доступного кэша расшифровать записи невозможно; при их утрате можно только уничтожить локальное хранилище и начать заново.": "The master password is never sent to the developer. When the client supports quick unlock, it is stored through Telegram SecureStorage using the device’s protected storage. Manual locking disables quick unlock until the next explicit password entry. Without the master password or an available cache, entries cannot be decrypted; if both are lost, you can only destroy the local vault and start over.",
  "Удаление": "Deletion",
